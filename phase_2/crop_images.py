...
def remove_likely_edge_components(mask: np.ndarray) -> np.ndarray:
    ...
    for label in range(1, component_count):
        ...
        # Scanner borders are often broken into strips that do not span half
        # the image. A quarter-width/height threshold removes those fragments
        # while the thinness and edge-touch requirements protect handwriting.
        thin_horizontal = h <= max(12, height * 0.05)
        thin_vertical = w <= max(12, width * 0.05)
        long_horizontal = thin_horizontal and (
            w >= width * 0.25 or w >= h * 12
        )
        long_vertical = thin_vertical and (
            h >= height * 0.25 or h >= w * 12
        )
...
def apply_crop(
    image: np.ndarray,
    box: CropBox | None,
    final_border_clip: int = FINAL_BORDER_CLIP,
) -> np.ndarray:
    """Apply the detected crop and optionally remove the remaining border."""
    cropped = image if box is None else image[
        box.top : box.bottom,
        box.left : box.right,
    ]
    if final_border_clip == 0:
        return cropped
...
def process_png(
    ...
    try:
        ...
        cropped = apply_crop(
            image,
            box,
            final_border_clip=settings.final_border_clip,
        )
    ...
    clip_description = (
        f"clipped {settings.final_border_clip}px border"
        if settings.final_border_clip
        else "final border clip skipped"
    )
    if box is None:
        logger.info(
            "Processed %s -> %s "
            "(no content crop; %s, output: %sx%s)",
            path,
            output_path,
            clip_description,
            cropped.shape[1],
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=f"Skip the final {FINAL_BORDER_CLIP}px clip after content cropping",
    )
...
def main() -> int:
    ...
    settings = CropSettings(
        ...
        final_border_clip=0 if args.skip_final_border_clip else FINAL_BORDER_CLIP,
    )
